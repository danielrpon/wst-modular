        ...
        for linea in reader:
            ...
            if linea["fuente"] == "jumbo":
                ...
                while productos_acumulados < total_productos:
                    ...
                    time.sleep(2)
                    driver.execute_script(
                        "window.scrollTo({top:document.body.scrollHeight-2000, left:0, behavior:'smooth'});"
                    )
            ...
            # Fuente Metrocol
            if linea["fuente"] == "metrocol":
                print(
                    f"Procesando Archivo {linea['busqueda']} en la fuente {linea['fuente']}"
                )
                # Navigate to url
                # https://www.tiendasjumbo.co/vogue%20polvo%20compacto%20natural?_q=VOGUE%20POLVO%20COMPACTO%20NATURAL&map=ft
                # https://www.tiendasmetro.co/pantene?_q=pantene&map=ft

                driver.get(
                    f"https://www.tiendasmetro.co/{linea['busqueda']}?_q={linea['busqueda']}&map=ft"
                )
                ...
                while productos_acumulados < total_productos:
                    ...
                    time.sleep(2)
                    driver.execute_script(
                        "window.scrollTo({top:document.body.scrollHeight-2900, left:0, behavior:'smooth'});"
                    )
                ...
                for product in products:
                    ...
                    # Precios
                    precios = product.find_all(
                        "div", class_="tiendasjumboqaio-metro-minicart-2-x-price"
                    )
                    if len(precios) > 1:
                        high_price = precios[0].text.replace(".", "")[2:]
                        low_price = precios[1].text.replace(".", "")[2:]
            ...
            if linea["fuente"] == "metroper":
                ...
                # https://www.tiendasjumbo.co/vogue%20polvo%20compacto%20natural?_q=VOGUE%20POLVO%20COMPACTO%20NATURAL&map=ft
                # https://www.tiendasmetro.co/pantene?_q=pantene&map=ft
                # https://www.metro.pe/busca/?ft=pantene
                driver.get(f"https://www.metro.pe/busca/?ft={linea['busqueda']}")

                # Esperar hasta que se tenga el total de botones cargados
                try:
                    WebDriverWait(driver, 20).until(
                        expected_conditions.visibility_of_all_elements_located(
                            (By.CLASS_NAME, "product-item product-item")
                        )
                    )
                except TimeoutException:
                    print("Se ha demorado mucho en cargar Inicial")
                ...
                try:
                    total_productos = int(
                        driver.find_elements(By.CLASS_NAME,"amount").text.split(" ")[0]
                    )
                except NoSuchElementException:
                    total_productos = 0
                    print("No se han encontrado resultados")
                ...
                while productos_acumulados < total_productos:
                    ...
                    time.sleep(2)
                    driver.execute_script(
                        "window.scrollTo({top:document.body.scrollHeight-2500, left:0, behavior:'smooth'});"
                    )
                ...
                for product in products:
                    # print(product)
                    # Imagen
                    picture = product.find_all("a", class_="product-item__image-link")[
                        0
                    ].attrs["href"]
                    # print(picture)
                    # SKU
                    ...
                    high_price = product.find_all(
                        "span",
                        class_="product-prices__value product-prices__value--best-price",
                    )[0].text.replace("S/", "")
                    low_price = product.find_all(
                        "span", class_="product-prices__value"
                    )[0].text.replace("S/", "")
                    ...
                    product_name = product.find_all("a", class_="product-item__name")[
                        0
                    ].attrs["title"]
            ...
            if linea["fuente"] == "wong":
                ...
                # Scrapper
                total_productos = driver.find_element(
            ...
            if linea["fuente"] in ["mercalicol", "mercaliper", "mercaliecu"]:
                ...
                total_productos = int(
                    driver.find_elements(
                        By.CLASS_NAME, "ui-search-search-result__quantity-results"
                    )
                    .text.split(" ")[0]
                    .replace(".", "")
                )
            ...
            if linea["fuente"] == "tia":
                ...
                try:
                    ...
                    print("Se ha demorado mucho en cargar Inicial")
                # Scrapper
                total_productos = driver.find_element(By.CSS_SELECTOR,".vtex-search-result-3-x-totalProducts--layout").text.split(" ")[0]
                if not total_productos:
                    total_productos = driver.find_element(By.CSS_SELECTOR,"#totalProducts2").text
                productos_acumulados = 0
                products = []
                # iterar hasta acumular el total de productos
            ...
            if linea["fuente"] == "fybeca":
                ...
                while has_products:
                    ...
                    try:
                        ...
                        # )
                        # ANTES WAIT POR 20
                        # document.getElementsByClassName('html-slot-container')[0].textContent
                    ...
                    has_products = (
                        not len(
                            soup.find_all("p", class_="no-results-message text-center")
                        )
                        > 0
                    )
                    ...
                    for product in products:
                        ...
                        # Nombre
                        name = (
                            product.find_all("img", class_="tile-image")[0]
                            .attrs["title"]
                            .strip()
                        )
                        ...
                        # Precio
                        high_price = (
                            product.find_all("span", class_="price-original")[0]
                            .text.strip()
                            .strip("(Antes)")
                            .strip("$")
                            .strip("\n")
                        ...
                        low_price = (
                            product.find_all("div", class_="large-price")[0]
                            .text.strip()
                            .strip("Oferta Club")
                            .strip("(Oferta)")
                            .strip("$")
                        ...
                        if len(low_price) == 0:
                            low_price = (
                                product.find_all("span", class_="value")[0]
                                .text.strip()
                                .strip("Oferta Club")
                                .strip("(Oferta)")
                                .strip("$")
                        ...
                        # condicional para cuando no encuentra producto
                        # print(driver.execute_script("document.getElementsByClassName('no-results-message text-center')[0].textContent"))
                        # valpru_ = driver.execute_script("document.getElementsByClassName('no-results-message text-center')[0].textContent")
                        # print('primero ',valpru_)
                        # try:
                        #     valpru_ = driver.execute_script("document.getElementsByClassName('no-results-message text-center')[0].textContent")
                        #     print('segundo ',valpru_)
                        #     if valpru_ == 'Lo sentimos, no encontramos resultados para tu búsqueda.':
                        #         has_products == False
                        #     else:
                        #         print('entro al if')
                        # except KeyError:
                        ...
                        writer.writerow(
                            {
                                ...
                                "brand": brand,
                                "product_name": name.title(),
                                "precio_normal": high_price.strip().strip("$").strip(),
                                "precio_descuento": low_price.strip()
                                .strip("$")
                                .strip(),
                                "imagen": picture[0].attrs["src"],
                            }
            ...
            if linea["fuente"] == "inkafarma":
                ...
                try:
                    total_productos = int(
                        driver.find_element(By.TAG_NAME,"h3").text.split(" ")[1]
                    )
                except NoSuchElementException:
                    total_productos = 0
                    print("No se han encontrado resultados")
            ...
            if linea["fuente"] == "plazavea":
                ...
                try:
                    total_productos = int(
                        driver.find_element(By.CSS_SELECTOR,".Search__content__count__total"
                        ).text
                    )
                except NoSuchElementException:
                    total_productos = 0
            ...
            if linea["fuente"] == "D1":
                ...
                # Scrapper
                total_productos = (
                    driver.find_elements(By.CLASS_NAME,"category-text")
            ...
            if linea["fuente"] == "ARA":
                ...
                # Scrapper
                total_productos = int(
                    driver.find_elements(By.CLASS_NAME,"picked-title")
                    .text.strip()
                    .split(" ")[0]
                )
